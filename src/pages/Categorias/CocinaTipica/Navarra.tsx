    ...
    const fetchRecipes = async () => {
      try {
        ...
        const recipesData = querySnapshot.docs
          ...
          .filter((recipe) => recipe.provincia === "Navarra"); //Filtrar las recetas por provincia
  ...
  return (
        <IonPage id="main-content" className="main-page">
          <IonHeader className="custom-header"> {/*Header del componente que incluye el menú desplegable*/}
            ...
            <IonTitle className="main-title">Navarra</IonTitle> {/*Título del componente*/}
              <IonMenuButton slot="start" />
            </IonToolbar>
          </IonHeader>
          <div id='contentNavarra'> {/*Contenido del componente con un mapa de recetas filtradas*/}
            <IonContent id="contentNavarra">
            {recipes.map((recipe) => (
              <RecipeCard
                key={recipe.id}
                recipe={recipe}
