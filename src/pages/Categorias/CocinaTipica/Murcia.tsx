    ...
    const fetchRecipes = async () => {
      try {
        ...
        const recipesData = querySnapshot.docs
          ...
          .filter((recipe) => recipe.provincia === "Murcia"); //Filtrar las recetas por provincia
  ...
  return (
        <IonPage id="main-content" className="main-page">
          <IonHeader className="custom-header"> {/*Header del componente que incluye el menú desplegable*/}
            ...
            <IonTitle className="main-title">Murcia</IonTitle> {/*Título del componente*/}
              <IonMenuButton slot="start" />
            </IonToolbar>
          </IonHeader>
          <div id='contentMurcia'> {/*Contenido del componente con un mapa de recetas filtradas*/}
            <IonContent id="contentMurcia">
            {recipes.map((recipe) => (
              <RecipeCard
                key={recipe.id}
                recipe={recipe}
