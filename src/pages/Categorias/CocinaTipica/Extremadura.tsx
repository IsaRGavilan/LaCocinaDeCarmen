    ...
    const fetchRecipes = async () => {
      try {
        ...
        const recipesData = querySnapshot.docs
          ...
          .filter((recipe) => recipe.provincia === "Extremadura"); //Filtrar las recetas por categoría
  ...
  return (
        <IonPage id="main-content" className="main-page">
          <IonHeader className="custom-header"> {/*Header del componente que incluye el menú desplegable*/}
            ...
            <IonTitle className="main-title">Extremadura</IonTitle> {/*Título del componente*/}
              <IonMenuButton slot="start" />
            </IonToolbar>
          </IonHeader>
          <div id='contentExtremadura'> {/*Contenido del componente con un mapa de recetas filtradas*/}
            <IonContent id="contentExtremadura">
            {recipes.map((recipe) => (
              <RecipeCard
                key={recipe.id}
                recipe={recipe}
