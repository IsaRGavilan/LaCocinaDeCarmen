import { useHistory } from 'react-router-dom'; //Importa useState para manejar el estado local del componente
import { IonContent, IonHeader, IonPage, IonTitle, IonToolbar, IonText, IonIcon } from '@ionic/react'; //Importa componentes Ionic
import { arrowBackOutline } from 'ionicons/icons'; //Importa icono usado
import './Terminos.css'; //Importa archivo de estilos
...
const Terminos = () => {
  ...
  return (
    <IonPage className='terminos'>
      <IonHeader>{/*Cabecera del componente*/}
        <IonToolbar className='cabecera-terminos' onClick={handleGoBack}>
            <IonIcon icon={arrowBackOutline} id='icono'/>{/*Icono clicable para volver a la pantalla anterior*/}
          <IonTitle className='titulo-terminos'>La Cocina de Carmen</IonTitle>{/*Título*/}
        </IonToolbar>
      </IonHeader>
      <IonContent>{/*Contenido del componente*/}
        <IonText>
